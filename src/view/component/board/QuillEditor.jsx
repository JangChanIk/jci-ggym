...
const QuillEditor = ({ value, handleContent, quillRef, files, handleFiles, trans}) => {

    const dispatch = useDispatch();

    const imageHandler = useCallback(() => {
        if(trans) return dispatch(setToastMsg("양도게시판에서는 이미지 업로드를 할 수 없습니다."));
        console.log(files);
        if(files.length > 2){
            return dispatch(setToastMsg("이미지는 3장까지 업로드 가능합니다."));
        }
        ...
        input.onchange = async () => {
            ...
            if(!fileType[fileType.length-1].toUpperCase().match('JPG')&&
                ...
                return dispatch(setToastMsg("jpg png jpeg형식만 지원합니다."));
            }
            ...
            if (!res.data) {
                return dispatch(setToastMsg("이미지 업로드에 실패하였습니다."));
            }
            const url = process.env.REACT_APP_SPRING_IP+`board/imageGet?imageName=${res.data}`;
    ...
    }, [quillRef, files, trans, dispatch, handleFiles]);
    ...
    return (
        <div style={{height: "850px", display: "flex", justifyContent: "center", padding:"0px"}}>
            <ReactQuill 
                ref={quillRef}
