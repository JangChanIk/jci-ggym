...
const NoticeListPage = ({id, page}) => { 
    ...
    return (
      <tr key={index} onClick={()=>{navigate(`/board/detail?id=${id}&bno=${listItem.bno}&page=${page}`)}}>
        { Object.keys(listItem).map((key, index) => (
          ...
          :
          key==='title'?
          <td key={index}>{<span><i style={{width:"15px", height:"15px"}} className={listItem.file?"fas fa-image":"fas fa-file-lines"}></i></span>}
          &nbsp;&nbsp;{listItem[key]}</td>
          :
  ...
  return (
    <>
      <ContainerDiv>
        <HeaderDiv>
          <h3 style={{marginLeft:"10px"}}>공지사항 게시판</h3>
        </HeaderDiv>
        <FormDiv>
          <div>
