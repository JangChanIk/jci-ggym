          ...
          <Card className="text-center" style={{width:"90%", maxWidth:"1200px"}}>
            ...
            <Card.Body>
              ...
              <div style={{display:"flex", flexWrap: "wrap", width:"100%", justifyContent: "space-around", height:"400px", marginTop:"30px"}}>
                {
                  payInfo.map((order, index) => (
                    <div key={index} style={{display: 'flex', flexDirection: 'column',justifyContent: 'space-between', maxWidth:"300px", width: "100%", height:"150px", margin:"20px"}}>
                      <div style={{display: 'flex', justifyContent: 'space-between'}}>
                        <span>주문번호 : </span><span>NO.{order.ORDER_NO}</span>
                      </div >
                      <div style={{display: 'flex', justifyContent: 'space-between'}}>
                        <span>상품이름 : </span><span>{order.PROD_NAME}</span>
                      </div>
                      {
                        order.PASS_CNT!==0&&
                        <div style={{display: 'flex', justifyContent: 'space-between'}}>
                          <span>남은 횟수 : </span><span>{order.PASS_CNT}</span>
                        </div>
                      }
                      <div style={{display: 'flex', justifyContent: 'space-between'}}>
                        <span>시작일 : </span><span>{order.PASS_SDAY}</span>
                      </div>
                      <div style={{display: 'flex', justifyContent: 'space-between'}}>
                        <span>만료일 : </span><span>{order.PASS_EDAY}</span>
                      </div>
                      <div style={{display: 'flex', justifyContent: 'space-between'}}>
                        <span>결제금액 : </span><span>{order.PAY_PRICE} 원</span>
                      </div>
                    </div>
                  ))
                }
